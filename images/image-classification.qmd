...
In this example we're going to use **zero-shot image classification** to see whether a painting is of a woman or a man. It's called "zero-shot" because we aren't customizing a model specifically for the task, but relying on a [pre-trained vision model](https://huggingface.co/openai/clip-vit-large-patch14) to know the difference between the two.
...
Finally, we (roughly) just use the rest of the example from [the documentation](https://huggingface.co/openai/clip-vit-large-patch14). Those model pages are a *delight* when their code works right out of the box.
